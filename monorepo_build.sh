...
for PARAM in $@; do
    ...
    # Rewrite all branches from the first remote, only develop branches from others
    if [ "$PARAM" == "$1" ]; then
        ...
        git checkout --detach $REMOTE/develop
...
for REF in $MERGE_REFS; do
    # Add all files from all develop branches into index
